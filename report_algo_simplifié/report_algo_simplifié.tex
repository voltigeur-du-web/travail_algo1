...
%     commentstyle=\itshape, % Style simple pour les commentaires,
%     escapeinside={/*@}{@*/}
% }
...
\section{Spécification de l'algorithme}
...
\begin{flalign}
  ...
  i \mapsto j \overset{def}{=} \label{eq:mapsto} & 0\leq w[i][j] < +\infty \lor 0 \leq w[j][i] < +\infty\\
  G\_non\_oriente(n) \overset{def}{=} \label{eq:nonoriente} &
  \left\{ \begin{aligned}
  & true & \text{ Si }\\
  & &\forall i,j \:|\: In\_graph(n, i) \land In\_graph(n, j) \land i \neq j :\\
  & & i \mapsto j \implies j \mapsto i\\ 
  & false & \text{ Sinon }\\
  \end{aligned}\right.
\end{flalign}
...
\begin{flalign}
  Chemins\_possibles(w,s,d)  \overset{def}{=} \label{eq:chemins_possibles} & \left\{ \begin{aligned}
    & \forall p[0..q-1] | \\ 
    &  \quad 0< q \leq n \land p[0] = s \land p[q-1] = d \land \\
    &  \quad  \forall k | 0 \leq k < q-1 \land 0 \leq w[k][k+1] < + \infty \\
    \end{aligned} \right.\\
...
\subsection{Démonstration des fonctions intermédiaires}
\subsubsection{Preuve de create\_pq}
Son environement : 
\begin{lstlisting}
...
\pre \equiv & \state{
  & 0 < n\_nodes \land \\
  & leq(a,b) \ \overset{def}{=}  \left\{ \begin{aligned}
    & x | x\leq 0 & \text{si }a \leq b  \\
    & x | x>0 & \text{sinon} \\
  \end{aligned} \right.
} \\
...
\subsubsection{Preuve de $is\_pq\_full$}
Son environement :
\begin{lstlisting}
...
\begin{flalign*}
  ...
  &\\
    ...
    \state{
      & pq = PQ\_init(leq,n) \land \\
      & tmp = Pointeur(struct Node_PQ) \land \\
    ...
    \Rightarrow \post\\
\end{flalign*}
\subsubsection{Preuve de $order\_lineage\_up$}
Son environement :
\begin{lstlisting}
struct PQ {
    Node_PQ_t list;
...
\post \equiv & \state{
  ...
  & Order\_up(pq,child,parent) = true \implies \\
        & \left\{ \begin{aligned}
        & Priority(pq,parent)=Priority(pq,parent_0) \land  & \text{si }Priority(pq,parent_0) \leq Priority(pq,child_0)  \\
...
\subsubsection{Preuve de $add\_to\_pq$}
Son environement :
\begin{lstlisting}
struct PQ {
    Node_PQ_t list;
...
\pre \equiv & \state{
  ...
  & \forall i,  0 \leq i < pq\to size \implies pq\to list[i].id \neq id \\
}\\
& \\
\post \equiv & \state{
  & pq = PQ\_init(leq,n) \land \\
...
\begin{flalign*}
  & \ppsp{
    ...
    & \forall i,  0 \leq i < pq_0\to size \implies pq_0\to list[i].id \neq id \\
  }
  } \\
  &\\
  & \equiv \ppsp{
    ...
    & 0 \leq priority \land \\
    & pq_0\to size < pq_0\to capacity \land\\
    & \forall i,  0 \leq i < pq_0\to size \implies pq_0\to list[i].d \neq id \land\\
...
\begin{flalign*}
  I \equiv & \state{
    ...
    & 0 \leq priority \land \\
    & pq_0\to size < pq_0\to capacity \land\\
    & \forall k,  0 \leq k < pq_0\to size \implies pq_0\to list[k].id \neq id \land\\
    & pq\to size = pq_0\to size+1  \land\\
...
Définissons d'abord deux ensembles de noeuds : $E_1$ pour l'ensemble des noeuds qui sont connectés à $s$ grâce
...
\end{aligned}
\right.&&\\
E_1 \overset{def}{=}& \state{k \in \mathbb{N} \mid 0 \leq k < n \land Connecte(n,s,k) = true}&&\\
E_2 \overset{def}{=}& \state{k \in \mathbb{N} \mid 0 \leq k < n \land explored[k] = \text{true}}&&\\
Reste(explored,d,E_1,E_2) \overset{def}{=}&
\left\{
\begin{aligned}
...
Nous sommes assurés que la boucle se termine car $f(\overline{x})$ décroit à chaque itération.
...
Faisons la liste des différents éléments significatifs qui interviennent dans l'algorithme :
\begin{itemize}
  \item Les paramètres $n$, $s$, $d$ de complexité spatiale constante.
  \item Le tableau à deux dimensions $w[0..n-1][0..n-1]$ de complexité spatiale $n^2$.
  \item Le tableau $path$ peut avoir une complexité spatiale jusqu'à $n$.
...
\end{itemize}
Dès lors, la complexité spatiale dans le pire des cas est : $T(n) = O(n^2)$ 
...
Soit $y$, le paramètre d'induction :


\begin{flalign*}
y \overset{def}{=}& pq\to size\\
E(y) \overset{def}{=}& \state{ i \in \mathbb{N} \:|\: 0 \leq i \leq n} \\
\end{flalign*}

Dès lors, 0 est la valeur minimal du paramètre d'induction. Lorsque $is\_pq\_empty(pq) = true$, 
...
Avant un retravail complet de l'algorithme en utilisant des structures moins complexes, 
j'ai souhaité répondre à un problème concret. 
Quel est le chemin le plus court entre un 2 points de mon village situé sur un croisement ? 
Un des buts de cette exercice était d'utiliser python avec le module osmnx et ctypes afin d'obtenir
des données disponibles d'open street map et d'utiliser un algorithme dévelopé en C pour générer
...
Plus d'information est accessible dans un \href{https://www.example.com}{repository github} temporairement public.
