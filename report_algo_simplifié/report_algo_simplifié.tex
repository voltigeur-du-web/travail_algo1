...
\begin{flalign}
  ...
  %   Parcours(w,p,\#p,s,d) \: | \: \forall p \in Chemins\_possibles(w,s,d) : \\
...
\begin{flalign*}
  \post \equiv & \state{
    ...
    &\state{
      &Chemins\_possibles(w,s,d) \neq \emptyset \implies \\
      & \state{
        & path[0..m-1] \text{ initialisé} \land 0 \leq m \leq n \land\\
        & path[0] = s \land\\
        & path[m-1] = d \land\\
        & \forall p \in Chemins\_possibles(w,s,d) : Parcours(w,path,m,s,d) \leq Parcours(w,p,m,s,d) \land \\ 
        & shortest\_path = \#path \\
      }\\
    } \land\\
    & Chemins\_possibles(w,s,d) = \emptyset \implies shortest\_path = 0\\  
} 
\end{flalign*}  
...
Soit
\begin{flalign}
  & a = Pointeur(S)  \overset{def}{=} \label{eq:pointeur} valide(a) \land alloue(sizeof(S)) &&\\
  & pq = PQ\_init(leq,n) \overset{def}{=}  
      \state{
      &pq = \text{Pointeur(struct PQ)} \land\\
      &pq \to \text{size} = 0 \land \\
      &pq \to \text{capacity} = n \land \\
      &pq \to \text{leq}(a,b) = \forall a,b \left\{ \begin{aligned}
        & pq \to \text{leq}(a,b)\leq 0 & \text{si }a \leq b  \\
        & pq \to \text{leq}(a,b)>0 & \text{sinon} \\
      \end{aligned} \right. \land \\
      &pq \to \text{list[0..n-1] initialisé}
      }&\label{eq:pqinit}\\
\end{flalign}
...
\begin{flalign*}
  \pre \equiv & \state{
    & 0<n\_nodes \land \\
    & \text{leq}(a,b) = \forall a,b \left\{ \begin{aligned}
        & pq \to \text{leq}(a,b)\leq 0 & \text{si }a \leq b  \\
        & pq \to \text{leq}(a,b)>0 & \text{sinon} \\
      \end{aligned} \right. \land \\
  } 
...
\begin{flalign*}
  \post \equiv & \state{
    & create\_pq = PQ\_init(leq, n\_nodes)\land\\
    & pq \to \text{size} = 0 \\
  } 
\end{flalign*}

\subsubsection{Démonstration}
Démonstration par la plus forte postcondition:
\begin{flalign*}
  & \ppsp{
...
\begin{flalign*}
\post \equiv & \state{
  & is\_pq\_full = \left\{ \begin{aligned}
...
\begin{flalign*}
\post \equiv & \state{
  &is\_pq\_empty = \left\{ \begin{aligned}
...
\begin{flalign}
  ...
  %   \end{aligned}
  % \right.\\
  &Order\_up(pq,parent) \overset{def}{=}
  \label{eq:pqupdecreasing}
  \state{
    &\forall k \:|\: 0\leq parent<k<pq\to size : \\
    &\quad pq\to leq(pq\to list[(k-1)/2].priority,
    pq\to list[k].priority)\leq 0
  }
  %   \left\{
  %   \begin{aligned}
  %     & true & \text{ Si } \state{
...
\post \equiv & \state{
  & pq = PQ\_init(leq,n) \land \\
  & Order\_up(pq,(child-1)/2)\land\\
  & order\_lineage\_up = (child-1)/2\\
} \\
\end{flalign*}
\subsubsection{Démonstration}
Démonstration par la plus forte postcondition :
\begin{flalign*}
  ...
  } \land \state{&pq->leq(pq->list[parent].piority,\\&\quad pq->list[child].piority)>0} \lor\\
  &\\
   &\ppsp{
    \parbox{0.3\linewidth}{
      \lstinline| return parent;|\\
  ...
  } \land \state{&pq->leq(pq->list[parent].piority,\\&\quad pq->list[child].piority)\leq 0} \\
    \equiv &\state{
        & pq = PQ\_init(leq,n) \land \\
        & 0 \leq child < pq\to size \land \\
        ...
        &\state{
          &\state{
              &\state{&pq->leq(pq->list[parent].piority,\\&\quad pq->list[child].piority)>0}\land\\
              & child = \text{Pointeur(struct Node\_PQ)} \land \\
              & parent = \text{Pointeur(struct Node\_PQ)} \land \\
              & child\to priority = parent_0\to priority \land child\to id = parent_0\to id \land \\
              & parent\to priority = child_0\to priority \land parent\to id = child_0\to id \land\\
              & order\_lineage\_up = parent\\
          }\lor\\
          &\state{
              &\state{&pq->leq(pq->list[parent].piority,\\&\quad pq->list[child].piority)\leq0}\land\\
              & order\_lineage\_up = parent\\
          }
        }
    } \\
...
\begin{flalign*}
  \pre \equiv & \state{
    ...
    & \forall j \in \mathbb{N} \:|\:  0 \leq j < pq_0\to size : pq_0\to list[j].id \neq id\\
  } 
\end{flalign*}
\subsubsection{Postcondition}
...
\post \equiv & \state{
  ...
  & \exists i , 0 \leq i < pq\to size : pq\to list[i].priority = priority \land pq\to list[i].id = id \\
}
\end{flalign*}
...
\begin{flalign*}
  & \ppsp{
    \parbox{0.3\linewidth}{
      ...
      \state{
    & pq_0 = PQ\_init(leq, n)\land\\
    & Order\_up(pq_0,0) \land\\
    & priority \in \mathbb{R}\land\\
    & id \in \mathbb{Z}\land\\
    & pq_0\to size < pq_0\to capacity \land\\
    & \forall j \in \mathbb{N} \:|\:  0 \leq j < pq_0\to size : pq_0\to list[j].id \neq id\\
  }
  } \\
  &\\
  & \equiv \ppsp{
    ...
    & priority \in \mathbb{R}\land\\
    & id \in \mathbb{Z}\land\\
    & pq\to size \leq pq_0\to capacity \land\\
    & \forall j \in \mathbb{N} \:|\:  0 \leq j < pq_0\to size : pq_0\to list[j].id \neq id\land\\
...
& \equiv 
    ...
    & i \in \mathbb{Z}\land i=pq\to size-1 \\
  }\\
\end{flalign*}
...
\begin{flalign}
  ...
  x = Prioritaire(pq)  \overset{def}{=}& \label{eq:prioritaire} 
    x \in \state{
      & pq\to list[j] \:|\: 0\leq j <pq\to size \land\\
      &\state{&\forall i \:|\: 0\leq j <pq\to size : \\
      & \quad pq\to leq(pq\to list[j]\to priority,
      pq\to list[i]\to priority)\leq 0 \\
    }}&&\\
...
\begin{flalign*}
  \post \equiv & \state{
    ...
    & Order\_up(pq,0) \land\\
    & serve\_pq = \text{Pointeur(struct PQ)}
  } 
\end{flalign*}
\subsection{peek pq}
\subsubsection{Environnement}
...
\begin{flalign*}
  \post \equiv & \state{
    & Pointeur(pq\to list) \text{ désalloué de la mémoire}\land\\
...
\lstinputlisting[language=C]{../simplified_dijkstra_algo/src/pq.h} 

\subsubsection{Données abstraites sur les méthodes de l'algorithme (dijkstra.h)}
\lstinputlisting[language=C]{../simplified_dijkstra_algo/src/dijkstra.h}
...
\lstinputlisting[language=C]{../simplified_dijkstra_algo/src/pq.c}

\subsection{Exploration de noeud et fonction principale}
\lstinputlisting[language=C]{../simplified_dijkstra_algo/src/dijkstra.c}
...
Celui-ci se termine par une boucle qui est particulièrement intéressante pour la preuve de l'invariant.
\begin{lstlisting}
...
\begin{flalign}
  &Min(a,b)  \overset{def}{=} \left\{ \begin{aligned}
    ...
    & b & \text{sinon}
  \end{aligned} \right. &&\\
  % &Chemin\_inverse(previous\_nodes,n,s,i) \overset{def}{=} \label{eq:chemin_inverse} 
  % \left\{ \begin{aligned}
  %     & \emptyset & \text{si } previous\_node[i] = -1 \land i \neq s  \\
  %     & s & \text{si }i = s  \\
  %     & Chemin\_inverse(previous\_nodes,n,s,previous\_node[i]) & \text{Sinon} \\
  % \end{aligned} \right. &&\\
  % &path = List\_to\_table(list) \overset{def}{=} path[0..m-1] \text{ initialisée} \:|\: 
  %   list(l_1,...,l_m) \land \forall k \:|\: 0\leq k < m \land : path[k] = k_{k+1}\\
  % &Reverse(path,m) \overset{def}{=} \label{eq:reverse} \left\{ \begin{aligned}
  %   & path[k] & \text{si } k = 0  \\
  %   & path[m-1-k] & \text{Sinon} \\
  % \end{aligned} \right. &&\\
  % &Chemin(previous\_nodes,m,s,i) \overset{def}{=} \label{eq:chemin}  Reverse(Chemin\_inverse(previous\_nodes,n,s,i),m) &&\\
...
Ensuite, les parties clés de l'invariant attaient  à la mise à jour de 
$distance\_from\_source$ et $previous\_nodes$ qui permettent de garantir que le chemin le plus court 
est construit au fur et à mesure de l'exploration des noeuds.
...
\begin{flalign*}
  I \equiv & \state{
    ...
    & \forall v \:|\: 0\leq v<n, 
    \state{
      &explored[v] = true \implies distance\_from\_source[v] = min \: Parcours\_possibles(w, s, v)\land\\
      &explored[v] = false \land \state{\exists u: 0\leq u <n \land u \mapsto v \land explored[u]=true} \implies \\
      &\quad distance\_from\_source[v] = min 
      \state{
 		    &min \: Parcours\_possibles(w, s, u) + w[u][v]
      }
    }\land\\
    & \forall v \:|\: 0\leq v<n,\\
    &previous\_nodes[v] \in
    \state{
      &u \in \mathbb{N} \:|\: u \mapsto v\land 0 \leq u < n \land explored[u] = true\land\\
      &distance\_from\_source[v] = min \: Parcours\_possibles(w, s, u) + w[u][v] \\
    }\cup \state{-1}\\
  } 
\end{flalign*}  
...
\begin{flalign*}
  ...
  \equiv & \state{
      ...
      & dist\_from\_source[0..n-1] \text{ initialisé} \land \\
      & previous\_nodes[0..n-1] \text{ initialisé} \land \\
      & explored[0..n-1] \text{ initialisé} \\
  }\\
\end{flalign*}
Dans $INIT_2$, une boucle est utilisée pour définir des valeurs initiales pour les 
structures créés en mémoire à l'étape précédente.
Dès lors, nous pouvons déduire de façon trivial que les structures créés précédemment
...
\begin{flalign*}
 P_2 \equiv & \ppsp{
  ...
  explored[i] := false;\}|}
    ...
    & \\
    &\text{Comme la boucle permet d'initialiser tous les éléments de ces 3 tableaux à leurs valeurs initiales:}\\
    & \\
    \equiv &  \state{
      & 0 < n \land \\
      & 0 \leq s < n \land 0 \leq d < n \land \\
      ...
      & pq = PQ\_init(leq,n) \land  \\
      & dist\_from\_source[0..n-1] \text{ initialisé} \land \\
      &\forall i \:|\: 0\leq i<n : dist\_from\_source[i]=FLT\_MAX\land\\
      & previous\_nodes[0..n-1] \text{ initialisé} \land \\
      &\forall i \:|\: 0\leq i<n : previous\_nodes[i]=-1\land\\
      & explored[0..n-1] \text{ initialisé} \land \\
      &\forall i \:|\: 0\leq i<n : explored[i]=false\\
    }\\
    & \\
    & \text{Etant donné l'absence de modifications apportés sur $n$, $w$, $s$ et $d$ 
    jusqu'à présent dans $INIT_1$ et $INIT_2$:}\\
    & \\
    \equiv &  \state{
      & 0 < n \land \\
      & 0 \leq s < n \land 0 \leq d < n \land \\
...
\begin{flalign*}
  ...
  \equiv & \ppsp{
    ...
    \state{
      & Exploration\_init(n,w,s,d, path) = true\land\\
      &\forall i \:|\: 0\leq i<n \land i \neq s: dist\_from\_source[i]=FLT\_MAX\land\\
      &\forall i \:|\: 0\leq i<n : previous\_nodes[i]=-1\land\\
      &\forall i \:|\: 0\leq i<n : explored[i]=false\\
...
On retrouve bien $Exploration\_init(n,w,s,d,path)$ dans $P\{INIT\}$ et l'invariant.  
Pour $distance\_from\_source[s]$, les deux antécédants des implications dans l'invariant 
sont faux donc les implications sont vérifiées. 
Enfin $previous\_nodes[v]$ sont tous égaux à -1 conformémant à l'invariant.
...
\begin{flalign*}
  P_3 \equiv & \state{
      ...
      & distance\_from\_source[s] = 0.0 \land \\
      & s = Prioritaire(pq) \land \\
      & current\_node := peek\_pq(pq)\land\\
      & k = Id(s)\\
    }
   \\
   &\Rightarrow I 
...
IF_3 \equiv & 
  ...
  \right)\\
\end{flalign*}
Nous pouvons démontrer cette condition par plus forte postcondition :
% \begin{landscape}
\begin{flalign*}
 Q_1 \equiv &\ppsp{R}{
  \state{
...
\equiv &\state{
    ...
    & k \neq d \land\\
    & Order\_up(pq,0)\land \\
    & \forall j \:|\: 0 \leq j<i\leq n :
    \state{
      & explored[j] = false \land \\
      & w[k][j]\neq +\infty \land \\
      & dist\_from\_source[k] + w[k][j]<dist\_from\_source[j]  
    }\implies\\
    &\quad\state{
    & j\in pq\land\\
    & dist\_from\_source[j] = dist\_from\_source_[k] + w[k][j]\land\\
    & previous\_node[j] = k\\
    } 
}
\end{flalign*}
...
\begin{itemize}
  ...
  \item Grâce aux propriétés de la file de priorité ($k=Pioritiare(pq) \land Order\_up(pq,0)$), 
  et à l'implication de la mise à jour de $dist\_from\_source[j]$ et $previous\_nodes[j]$ dans $Q_3$ 
  lorsque la distance venant de la source des noeuds voisins est inférieure à la distance actuelle,
  nous pouvons conclure que les implications de l'invariant $I$ sont respectées:
  \begin{itemize}
    \item Si un noeud $v$ est exploré, sa distance depuis la source est minimale.
    \item Si un noeud $v$ n'est pas exploré mais a un voisin $u$ exploré, 
    sa distance depuis la source est $distance\_from\_source[v] = min (min \: Parcours\_possibles(w, s, u) + w[u][v])$.
    \item Le tableau $previous\_nodes$ est mis à jour correctement pour refléter les chemins optimaux découverts.
    \end{itemize}
  \end{itemize}
...
}
{
  \state{
    ...
    & \forall v \:|\: 0\leq v<n,\\ 
    &\quad\state{
      &explored[v] = true \implies\\ 
      & \quad distance\_from\_source[v] = min \: Parcours\_possibles(w, s, v)\land\\
      &explored[v] = false \land\\ 
      &\quad \state{\exists u: 0\leq u <n \land u \mapsto v \land explored[u]=true} \implies \\
      &\quad distance\_from\_source[v] = min\\ 
      &\quad \quad \state{
 		    &min \: Parcours\_possibles(w, s, u) + w[u][v]
      }
    }\land\\
    & \forall v \:|\: 0\leq v<n, previous\_nodes[v] \in\\
    &\quad\state{
      &u \in \mathbb{N} \:|\: u \mapsto v\land 0 \leq u < n \land explored[u] = true\land\\
      &distance\_from\_source[v] = min \: Parcours\_possibles(w, s, u)\\ 
      &\quad\quad + w[u][v] \\
    }\cup\\ 
    &\quad\state{-1}\\
  }
...
\begin{flalign}
  ...
  &Reverse(path,m) \overset{def}{=} \label{eq:reverse} \left\{ \begin{aligned}
    ...
    & path[m-1-k] & \text{Sinon} \\
  \end{aligned} \right. &&\\
  &Chemin(previous\_nodes,m,s,i) \overset{def}{=} \label{eq:chemin}  List\_to\_table(Reverse(Chemin\_inverse(previous\_nodes,n,s,i),m)) &&
\end{flalign}
...
\begin{flalign*}
I_2 \supseteq \state{&path\_n\_nodes = \#Chemin\_inverse(previous\_nodes,n,i\_node,d)\land\\
  & i\_node \in \mathbb{N} \:|\: 0 \leq i\_node < n}
\end{flalign*}

A la sortie de boucle, la condition suivante est incluse dans $I_2 \land \neg B_2$ :
 \begin{flalign*}
I_2 \land \neg B_2 \supseteq \state{path\_n\_nodes = \#Chemin\_inverse(previous\_nodes,n,s,d)}
\end{flalign*}
...
Q_6 \equiv &
   ...
   & Exploration\_init(n, w, s, d, path) = true \land\\
    ...
    &\state{
      ...
      &pq\to size > 0}\land\\
    & i\_node = s \land\\
    & path\_n\_nodes = \#Chemin\_inverse(previous\_nodes,n,s,d)\land\\
    & m = path\_n\_nodes\\
  } \\
&\\
\end{flalign*}
...
\begin{flalign*}
I_3 \supseteq \state{&my\_path = Chemin(previous\_nodes,m,i\_node,d)\land\\
  & i\_node \in \mathbb{N} \:|\: 0 \leq i\_node < n}
\end{flalign*}

Se faisant, la condition à la sortie de la boucle inclut la condition suivante :
\begin{flalign*}
I_3 \land \neg B_3 \supseteq \state{my\_path = Chemin(previous\_nodes,m,s,d)}  
\end{flalign*}
...
Q_7 \equiv & 
   ...
   & Exploration\_init(n, w, s, d, path) = true \land\\
    ...
    & my\_path[0..m-1] \text{ initialisé }\land\\
    & my\_path = Chemin(previous\_nodes,m,s,d)\\
  }\\
&\\
\end{flalign*}
...
\equiv & \ppsp{
   ...
   & Exploration\_init(n, w, s, d, path) = true \land\\
    ...
    & m = path\_n\_nodes\land\\
    & my\_path[0..m-1] \text{ initialisé }\land\\
    & my\_path = Chemin(previous\_nodes,m,s,d)\land\\
    & path = my\_path[0..m-1]\\
  } 
}\\
&\\
...
&\\
\end{flalign*}
Rappelons-nous de la postcondition de $shortest\_path$ en section~\ref{subsec:shortest_path} :
\begin{flalign*}
  \post \equiv & \state{
    & n = n_0 \land  w = w_0 \land \\
    & s = s_0 \land d = d_0 \land \\
...
\begin{itemize}
  ...
  \item Le premier cas prévu dans la postcondition $Chemins\_possibles(w,s,d)\neq \emptyset$ 
  correspond aux disjonctions de $Q_9$ où $shortest\_path >0$. Dès lors, on retrouve bien:
  \begin{itemize}
    \item $path[0..m-1]$ est initialisé.
    \item $path[0] = s$ (et $path[0] = d$ lorsque $s=d$).
    \item $path[m-1] = d$.
    \item $shortest\_path = m$.
    \item Pour $s\neq d$, le chemin retourné est bien le plus court car celui-ci a été construit
    à partir des $previous\_nodes$ où la propriété\eqref{prop:previous} ci-dessous est respectée.
  \end{itemize}
  \item Le second cas prévu dans la postcondition $Chemins\_possibles(w,s,d)= \emptyset$
  correspond à la disjonction de $Q_9$ où $shortest\_path = 0$. Dans ce cas, on retrouve bien:
  \begin{itemize}
    \item $shortest\_path = 0$.
  \end{itemize}
\end{itemize}
\begin{flalign}
  \state{
  & \label{prop:previous}\forall v \:|\: 0\leq v<n, previous\_nodes[v] \in\\
&\quad\state{
&u \in \mathbb{N} \:|\: u \mapsto v\land 0 \leq u < n \land explored[u] = true\land\\
&distance\_from\_source[v] = min \: Parcours\_possibles(w, s, u)\\ 
&\quad\quad + w[u][v] \\}\cup\state{-1}}
\end{flalign}
...
$log_2(n)$ 
\end{itemize}
Si nous faisons le total de ces différentes assignations, nous obtenons
$n * (n\ + log_2(n)) * 2 * log_2(n)$. 
La complexité temporelle dans le pire des cas est $T(n) = O(n^2)$.
...
\lstinputlisting[language=C]{../simplified_dijkstra_algo/src/dijkstra_recursive.c}
\section{Informations complémentaires}
Avant un retravail complet de l'algorithme, j'ai souhaité répondre à un problème concret que l'algorithme
de Dijkstra peut répondre. Quel est le chemin le plus court entre deux intersections de mon village ? 
...
des données disponibles d'open street map et d'utiliser un algorithme développé en C via python pour générer
